...
if __name__ == "__main__":
    ...
    shapes_df = pandas.read_csv(shapes_file)
    ...
    node_coords_df = pandas.read_csv(node_coords_file)
    ...
    access_egress_df = pandas.concat([access_df, egress_df])
    # rename the columns
    ...
    access_egress_df = pandas.merge(left=access_egress_df, right=node_coords_df, on="N", how="left")
    # drop N now we're done with it
    access_egress_df.drop("N", axis=1, inplace=True)
    # print len(access_egress_df)
    # print access_egress_df.head()

    print("Out of %d (non-unique) nodes, %d failed to find coordinates" % (len(access_egress_df), pandas.isnull(access_egress_df["shape_pt_lon"]).sum()))

    # append to shapes and save
    shapes_df = pandas.concat([shapes_df, access_egress_df])
